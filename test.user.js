...
*
* Newly added (v7.17):
* - Fixed the "Switch to video" hiding rule taking the enhance-badge (Smart
*   Shuffle recommendation indicator, data-testid="enhance-badge") down with it
*   whenever a track also had a music video. The badge and the video-switch
*   block are siblings inside the same outer wrapper, not parent/child - the
*   old selector hid that whole outer wrapper; it now targets the video-switch
*   block's own inner wrapper one level deeper, so the badge stays visible in
*   both expanded and compact modes regardless of whether a video exists.
* - Added explicit flex/alignment rules for the badge's row so a lone icon
*   (no video block next to it) doesn't get squashed or misaligned once the
*   compact-mode column layout (overflow:hidden + white-space:nowrap) applies
*   to it same as the title/artist rows.
* - Optional: added a compact-mode proxy for "Remove recommendation" -
*   Smart-Shuffle-only, only exists while the current track actually has that
*   button. Matched purely by its own aria-label, same reasoning as the
*   "Add to " proxy above - never reaches into their shared grouped-button
*   container class.
*
* Newly added (v7.18) - corrected v7.17's two rough edges:
* - The enhance-badge no longer gets any extra display/align/min-height CSS
*   of its own. That CSS was fighting the badge's native layout (visibly
*   misaligned against the artist-name row) instead of helping - the
*   no-video DOM already renders correctly with zero extra CSS from this
*   script, and that's the target appearance once the video block is
*   hidden in the with-video case too, so nothing extra is applied.
* - The Remove-recommendation proxy's position is no longer a guessed
*   static px value. ensureRemoveRecProxy() now measures the real, live
*   on-screen gap between the actual "Remove recommendation" and "Add to "
*   buttons via getBoundingClientRect() at the exact moment compact mode is
*   entered - while they're still visible, just before the .spf-compact
*   class hides their real container - and sets the proxy's right-offset
